...
- Feat: Haxe `.hx` + `.hxml` first-class AST extraction and cross-file import resolution. Full tree-sitter-based extraction: classes, enums, abstracts, typedefs, interfaces, generic type parameters, metadata annotations, access modifiers (`public`/`private`/`static`/`override`/`inline`/`dynamic`/`extern`), property `get`/`set` links, arrow functions, `switch`/`case` pattern labels, `cast`/`ECheckType` type refs, `EMeta` expression metadata, `$type` traces, object literal fields, `import` aliasing, conditional compilation (`#if`/`#else`/`#end`), and `.hxml` multi-target (`--next`) build-config extraction with `-lib`/`-cp`/`-main`/`-D`/`--interp` directives. Cross-file import resolution follows the Java pattern. 36 tests covering all extraction features.
